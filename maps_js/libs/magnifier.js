...
function Magnifier(width, height, zoomOffset ) {
	...
	var setOverview = function (obj, key, value, oldValue) {
		...
		if (key === "center") {
			...
			value = (value + that.zoomOffset >= that.__map2.minZoomLevel) ? value : that.__map2.minZoomLevel;
			that.__map2.set(key, value + that.zoomOffset);
		}
	};
	...
	var createSecondMap = function (map) {
				...
				that.__map2.set({ center: that.__map.get("center"), zoomLevel: 
					(that.__map.get("zoomLevel")+ that.zoomOffset) });
